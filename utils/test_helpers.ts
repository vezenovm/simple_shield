...
export function generateTestTransfers(num_transfers: number, barretenberg: BarretenbergSync) {
  let transfers = [];
  for (var i = 0; i < num_transfers; i++) {
    let sender_priv_key = Fr.fromBuffer(Buffer.from("000000000000000000000000000000000000000000000000000000616c696365", "hex"));
    let sender_public_key = barretenberg.schnorrComputePublicKey(sender_priv_key);
    ...
    let note_commitment = barretenberg.pedersenHash([sender_pubkey_x, sender_pubkey_y, secret], 0);

    let nullifier = barretenberg.pedersenHash([note_commitment, Fr.fromBuffer(Buffer.from(toFixedHex(i, false), 'hex')), sender_priv_key], 0);
